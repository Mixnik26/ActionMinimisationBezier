...
where the Hessian matrix is defined as $\left(H_{f}\right)_{ij} = \partial^2 L/\partial f_{i} \partial f_{j}$ for entries $i\ j$, and $\boldsymbol{\eta_i} = [\delta_i, \dot{\delta_i}]^{T}$.
